 ...
 */
public class SplitLayoutState(
    initialSplitFraction: Float,
) {
    /**
                ...
                Modifier
                    .run {
                        if (strategy.isHorizontal()) {
                            fillMaxHeight().width(draggableWidth)
                        } else {
                            fillMaxWidth().height(draggableWidth)
                        }
                    }.draggable(
...
private fun MeasureScope.notReadyLayout(constraints: Constraints) = layout(constraints.minWidth, constraints.minHeight) {}
...
private fun MeasureScope.doLayout(
    ...
    val dividerWidth = with(density) {
        2.dp.roundToPx() // dividerStyle.metrics.thickness.roundToPx()
    }
...
internal fun horizontalTwoPaneStrategy(gapWidth: Dp = 0.dp): SplitLayoutStrategy = object : SplitLayoutStrategy {
        ...
        return SplitResult(
            ...
            gapBounds =
                Rect(
                    ...
                    bottom = layoutCoordinates.size.height
                        .toFloat()
                        .coerceAtLeast(1f),
                ),
        )
...
internal fun verticalTwoPaneStrategy(gapHeight: Dp = 0.dp): SplitLayoutStrategy = object : SplitLayoutStrategy {
    override fun calculateSplitResult(
        density: Density,
        layoutDirection: LayoutDirection,
        state: SplitLayoutState,
        ...
        return SplitResult(
            ...
            gapBounds =
                Rect(
                    ...
                    right = layoutCoordinates.size.width
                        .toFloat()
                        .coerceAtLeast(1f),
